      ...
      this.each(function () {
         ...
         if (options.addable) {
            $(easyTree)
               ...
               .append('<div class="create"><button class="btn btn-default btn-sm btn-success"><span class="glyphicon glyphicon-plus"></span></button></div> ')
            $(easyTree)
                              ...
                              .click(function (e) {
                                 ...
                                 if (options.deletable || options.editable || options.addable) {
                                    ...
                                    if (options.editable) {
                                       ...
                                       else $(easyTree).find(".easy-tree-toolbar .edit > button").removeClass("disabled")
                                    }

                                    if (options.deletable) {
                                       if (selected.length <= 0 || selected.length > 1)
                                          $(easyTree).find(".easy-tree-toolbar .remove > button").addClass("disabled")
                                       else $(easyTree).find(".easy-tree-toolbar .remove > button").removeClass("disabled")
                                    }
                                 }
         ...
         $(easyTree).delegate("li.parent_li > span", "click", function (e) {
            ...
            if (children.is(":visible")) {
               children.hide("fast")
               $(this).attr("title", options.i18n.expandTip).find(" > span.glyphicon").addClass("glyphicon-folder-close").removeClass("glyphicon-folder-open")
            } else {
               children.show("fast")
               $(this).attr("title", options.i18n.collapseTip).find(" > span.glyphicon").addClass("glyphicon-folder-open").removeClass("glyphicon-folder-close")
            }
            e.stopPropagation()
         })
            ...
            $(easyTree)
               ...
               .click(function (e) {
                  ...
                  if (options.deletable || options.editable || options.addable) {
                     var selected = getSelectedItems()
                     if (options.editable) {
                        if (selected.length <= 0 || selected.length > 1) $(easyTree).find(".easy-tree-toolbar .edit > button").addClass("disabled")
                        else $(easyTree).find(".easy-tree-toolbar .edit > button").removeClass("disabled")
                     }

                     if (options.deletable) {
                        if (selected.length <= 0 || selected.length > 1) $(easyTree).find(".easy-tree-toolbar .remove > button").addClass("disabled")
                        else $(easyTree).find(".easy-tree-toolbar .remove > button").removeClass("disabled")
                     }
                  }
